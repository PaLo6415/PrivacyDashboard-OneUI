package rk.android.app.privacydashboard.activity.permission.log.database;
...
public interface LogsDao {
    ...
    @Query("SELECT * FROM logs_table ORDER BY startTimestamp DESC")
    LiveData<List<Logs>> getAllLogs();

    @Query("SELECT * FROM logs_table WHERE permission=:permission ORDER BY startTimestamp DESC")
    LiveData<List<Logs>> getAllLogsForPermission(String permission);

    @Query("SELECT * FROM logs_table WHERE packageName=:packageName ORDER BY startTimestamp DESC")
    LiveData<List<Logs>> getAllLogsForPackage(String packageName);

    @Query("SELECT * FROM logs_table WHERE permission=:date ORDER BY startTimestamp DESC")
    LiveData<List<Logs>> getAllLogsForDate(String date);

    @Query("SELECT * FROM logs_table WHERE logId=:id")
    Logs getLogFromId(int id);
